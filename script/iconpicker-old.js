; (function ($, document, undefined) {
    ...
    $(document).on("dialog-closed", function (event) {
        $('.informa-icon-picker').remove();
    });
    ...
    var bindField = function ($field, $popover, icons) {
        ...
        if ($(".icon-picker.coral-Popover.informa-icon-picker").find(">.picker-icons-header:nth-child(1)").length == 0 && $(".icon-picker.coral-Popover.informa-icon-picker").find(">.picker-icons-container:nth-child(1)").length == 0) {
        ...
        var $activator_icon = $('<coral-icon class="coral3-Icon coral3-Icon--sizeS coral3-Icon--filter" icon="filter" size="S" role="img" aria-label="Filter"></coral-icon>');
        var $activator_label = $('<coral-button-label />');
        $activator.append($activator_icon);
        $activator.append($activator_label);
        if ($(".coral-Form-fieldwrapper").find(">button").find($(".coral3-Icon--filter")).length == 0) {
